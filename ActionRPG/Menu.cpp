...
void Menu::event(SDL_Event& evt)
{
	...
	{
		if (evt.key.scancode == SDL_SCANCODE_DOWN)
		{
		...
		else if (evt.key.scancode == SDL_SCANCODE_RETURN || evt.key.scancode == SDL_SCANCODE_RIGHT)
		{
			Audio::instance()->playSound("kick");
			_items[_itemSelected]->run();
		}
		else if (_closable && (evt.key.scancode == SDL_SCANCODE_ESCAPE || evt.key.scancode == SDL_SCANCODE_LEFT))
		{
...
Menu* Menu::mainMenu()
{
	...
	menu->addItem("Options", [menu]()
	{
		...
		int vsync = 0;
		SDL_GetRenderVSync(Game::instance()->window()->renderer(), &vsync);
		...
		nestedMenu->addItem(std::string("VSYNC ") + (vsyncOn ? "on" : "off"), [nestedMenu]()
			{
				...
				SDL_SetRenderVSync(Game::instance()->window()->renderer(), !vsyncOn);
			});
