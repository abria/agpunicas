...
void RPGGameScene::event(SDL_Event& evt)
{
	...
	if (evt.type == SDL_EVENT_KEY_DOWN && evt.key.scancode == SDL_SCANCODE_RETURN)
	...
	else if (evt.type == SDL_EVENT_KEY_DOWN && evt.key.scancode == SDL_SCANCODE_A && !evt.key.repeat)
		link->attack();
	else if (evt.type == SDL_EVENT_KEY_DOWN && evt.key.scancode == SDL_SCANCODE_I && !evt.key.repeat)
		link->interact();
	else if (evt.key.scancode == SDL_SCANCODE_L && !evt.key.repeat)
	{
