	...
	while (_timeToSimulateAccum >= _dt)
	{
		...
		for (int i = 0; i < contactEvents.beginCount; ++i)
		{
			...
			if (!b2Shape_IsValid(beginEvent->shapeIdA) || !b2Shape_IsValid(beginEvent->shapeIdB) ||
				!b2Contact_IsValid(beginEvent->contactId))
				continue;
			...
			Vec2Df normal = { contactData.manifold.normal.x, contactData.manifold.normal.y };
		...
		for (int i = 0; i < contactEvents.endCount; ++i)
		{
			...
			RigidObject* objA = b2Shape_IsValid(endEvent->shapeIdA) ?
				((RigidObject*)b2Body_GetUserData(b2Shape_GetBody(endEvent->shapeIdA))) : nullptr;
			RigidObject* objB = b2Shape_IsValid(endEvent->shapeIdB) ?
				((RigidObject*)b2Body_GetUserData(b2Shape_GetBody(endEvent->shapeIdB))) : nullptr;
...
void ComplexPlatformerGameScene::event(SDL_Event& evt)
{
	...
	{
		if(evt.key.scancode == SDL_SCANCODE_RETURN || evt.key.scancode == SDL_SCANCODE_ESCAPE)
			Game::instance()->pushScene(Menu::pauseMenu());
		else if (evt.key.scancode == SDL_SCANCODE_SPACE && !evt.key.repeat)
			player->jump();
		else if (evt.key.scancode == SDL_SCANCODE_D && !evt.key.repeat)
			player->dash();
		else if (evt.key.scancode == SDL_SCANCODE_F && !evt.key.repeat)
			player->fire();
	}
