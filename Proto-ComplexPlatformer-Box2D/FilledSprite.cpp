	...
	: Sprite(spritesheet, rect)
{
	...
	if (_tileSize.x == 0 || _tileSize.y == 0)
		_tileSize = _rect.size / Game::instance()->tileSize();
}

void FilledSprite::render(SDL_Renderer* renderer, const RectF& drawRect, Transform camera, 
	float angle,			// rotation in degrees, clockwise
	SDL_RendererFlip flip)
{
	...
	if (angle)
	{
		std::cerr << "FilledSprite::render() -> rotation not supported for filled sprites\n";
		return;
	}
