...
// - draws scene objects into the parent window through a viewport
// - only scene objects within the view's rect are drawn (culling)
// - handles scene-view-window transforms
class agp::View
{
	private:
	...
	public:
		...
		void setViewport(const RectF& r) { _viewport = r; }
		void setFixedAspectRatio(float ratio) { _aspectRatio = ratio; }
		...
		void scale(float f);
		void setPos(const Vec2Df& newPos);

		// update viewport according to current renderer state
		void updateViewport();
