...
namespace agp
{
	...
	inline static void parse_range(const std::string & str, int &a, int &b, int &c, int &d) 
	{
		...
		if(!std::regex_match(str, rgx))
			throw (strprintf("\"%s\" does not match string pattern [a,b)\\[c,d)", str.c_str()));
		...
		a = astr == "inf" ? std::numeric_limits<int>::max() : agp::str2num<int>(astr);
		b = bstr == "inf" ? std::numeric_limits<int>::max() : agp::str2num<int>(bstr);
		c = cstr == "inf" ? std::numeric_limits<int>::max() : agp::str2num<int>(cstr);
		d = dstr == "inf" ? std::numeric_limits<int>::max() : agp::str2num<int>(dstr);
	}
}
